...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def accumulateTimes(remaining: List[Char], accumulated: List[(Char, Int)]): List[(Char, Int)] = {
      if (remaining.isEmpty) accumulated
      else {
        val current = remaining.head
        val currentCount = accumulated.find(_._1 == current).map(_._2)
        val updated: List[(Char, Int)] = currentCount match {
          case Some(count: Int) => accumulated.filterNot(_._1 == current) :+(current, count + 1)
          case None => accumulated :+(current, 1)
        }
        accumulateTimes(remaining.tail, updated)
      }
    }
    accumulateTimes(chars, List())
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def insert(newTree: CodeTree, trees: List[CodeTree]): List[CodeTree] = {
      @tailrec
      def recInsert(left: List[CodeTree], right: List[CodeTree]): List[CodeTree] = {
        if (right.isEmpty) left :+ newTree
        else if (weight(right.head) > weight(newTree)) (left :+ newTree) ++ right
        else recInsert(left :+ right.head, right.tail)
      }
      recInsert(List(), trees)
    }

    if (trees.isEmpty || singleton(trees)) trees
    else {
      val (tree1, tree2) = (trees.head, trees.tail.head)
      val (left, right) = if (weight(tree1) > weight(tree2)) (tree2, tree1) else (tree1, tree2)
      val newFork = Fork(left, right, chars(left) ++ chars(right), weight(left) + weight(right))
      val treesHeadRemoved = trees.tail.tail
      insert(newFork, treesHeadRemoved)
    }
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.find(entry => entry._1 == char) match {
      case Some((key, encoded)) => encoded
      case _ => List()
    }
  }
